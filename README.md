...
- **图片转 PDF**：可一次多选多个文件夹，或选择上级目录批量添加子文件夹；每个文件夹内的图片合并为一个 PDF，保存在各文件夹的上级目录
...
- 单个文件夹：按文件名中的**数字顺序**合并为一份 PDF，输出到**文件夹上级目录**，文件名为文件夹名（如 `D:/照片/我的资料/` → `D:/照片/我的资料.pdf`）；文件夹内的 PDF 文件会自动忽略
